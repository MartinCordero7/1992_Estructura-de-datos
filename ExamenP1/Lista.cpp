...
void Lista::filtrarPorPeriodo(int anioInicio, int anioFin)
{
    if (!cabeza)
    {
        cout << "La lista está vacía. No hay libros para filtrar." << endl;
        return;
    }
    ...
    {
        cout << "El rango de años no es válido. El año de inicio debe ser menor o igual al año final." << endl;
        return;
    }
    ...
    cout << "Libros publicados entre " << anioInicio << " y " << anioFin << ":" << endl;

    do
    {
        
        int anio = 0;
        size_t pos = actual->fechaPublicacion.rfind('/'); 
        if (pos != string::npos)
        {
            try
            {
                anio = stoi(actual->fechaPublicacion.substr(pos + 1));
            }
            catch (const invalid_argument &e)
            {
                cout << "Error al leer el año en el libro: " << actual->libro << endl;
                continue;
            }
        }
